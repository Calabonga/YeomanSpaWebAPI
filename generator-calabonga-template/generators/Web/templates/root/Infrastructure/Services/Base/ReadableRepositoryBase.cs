...
namespace YeomanTemplate.Web.Infrastructure.Services.Base {
    ...
    public abstract class ReadableRepositoryBase<TModel, TQueryParams> : IReadableRepository<TModel, TQueryParams>
        ...
        protected ReadableRepositoryBase(IAppContext context, IAppConfig config, IMapper mapper, IServiceSettings settings, ILogService logger) {
            Settings = settings;
            LogService = logger;
            Mapper = mapper;
        ...
        /// <returns></returns>
        public virtual OperationResult<TModel> GetById(Guid id, bool includes = false) {
            var item = includes
